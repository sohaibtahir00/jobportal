...
// Simplified registration schema without name field
...
export default function SignupPage() {
  ...
  const onSubmit = async (data: SignupFormData) => {
    if (!selectedRole) {
      showToast("error", "Role required", "Please select whether you're a candidate or employer");
      return;
    }
    ...
    if (selectedRole === "employer" && !isWorkEmail(data.email)) {
    ...
    try {
      // Call the real signup API - name will come from onboarding/resume
      await signup({
        email: data.email,
        password: data.password,
        fullName: data.email.split("@")[0], // Temporary placeholder, will be updated in onboarding
        role: selectedRole,
      });
  ...
  const handleGoogleSignup = async () => {
    if (!selectedRole) {
      showToast(
        "error",
        "Role required",
        "Please select whether you're a candidate or employer first"
      );
      return;
    }

    // Only candidates can use Google OAuth
    if (selectedRole === "employer") {
      showToast(
        "info",
        "Company email required",
        "Employers must sign up with their company email address"
      );
      return;
    }
    ...
    <div className="min-h-screen grid lg:grid-cols-2">
      ...
      <div className="flex items-center justify-center p-8 bg-gradient-to-br from-purple-50 via-white to-blue-50 overflow-y-auto">
        ...
        >
          ...
          {/* Step 2: Show options only after role is selected */}
          <AnimatePresence mode="wait">
            {selectedRole && (
              <motion.div
                ...
                className="space-y-4"
              >
                {/* OAuth Buttons - Based on Role */}
                {selectedRole === "candidate" ? (
                  <>
                    {/* Google Button - Candidates Only */}
                    <button
                      type="button"
                  ...
                  <>
                    {/* Employer: LinkedIn Only (Coming Soon) */}
                    <button
                      type="button"
                      onClick={handleLinkedInSignup}
                      className="w-full flex items-center justify-center gap-3 px-6 py-3 bg-white border-2 border-gray-300 rounded-xl font-semibold text-gray-700 hover:bg-gray-50 hover:border-gray-400 transition-all shadow-sm"
                ...
                {/* Email/Password Form - NO Name Field */}
