 ...
 * Proxy to backend GET /api/admin/analytics/revenue
 */
export async function GET(request: NextRequest) {
  try {
    ...
    const url = `${BACKEND_URL}/api/admin/analytics/revenue${queryString ? `?${queryString}` : ''}`;
    ...
    if (!response.ok) {
      const error = await response.json().catch(() => ({ error: 'Failed to fetch revenue analytics' }));
