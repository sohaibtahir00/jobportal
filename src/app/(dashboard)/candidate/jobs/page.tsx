...
function CandidateJobsContent() {
  ...
  // Get candidate profile data for skills assessment info
  const { profile } = useCandidateProfile();
  const hasCompletedAssessment = profile?.hasTakenTest || false;
  ...
  const queryParams: GetJobsParams & { exclusiveOnly?: boolean } = {
    ...
    exclusiveOnly: exclusiveOnly || undefined,
  };
        ...
        <div className="container">
          <div className="flex items-start justify-between mb-4">
            ...
            {hasCompletedAssessment && (
              <div className="hidden lg:flex items-center gap-2 bg-white/10 backdrop-blur-sm border border-white/20 rounded-lg px-4 py-3">
                <Award className="h-5 w-5" />
                <div className="text-sm">
                  <p className="font-semibold">Assessment Verified</p>
                  <p className="text-xs text-primary-100">Access to exclusive jobs</p>
                </div>
              </div>
            )}
          </div>
          ...
          {/* Search Bar */}
          <div className="max-w-4xl bg-white rounded-xl p-2 shadow-2xl mb-6">
          ...
          {!hasCompletedAssessment && (
            <div className="flex items-center gap-3 bg-yellow-500/20 backdrop-blur-sm border border-yellow-400/50 rounded-lg px-4 py-3">
              <Award className="h-5 w-5 text-yellow-100" />
              <p className="text-sm">
                <span className="font-semibold">Complete your Skills Assessment</span> to unlock 250+ exclusive jobs!
              </p>
              <Button size="sm" variant="secondary" asChild>
                <a href="/skills-assessment">Start Assessment</a>
              </Button>
            </div>
          )}
        </div>
